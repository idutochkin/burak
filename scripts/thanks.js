function loadFiles() {
  ...
  fileThanks.addEventListener("change", async function (e) {
    ...
    const response = await fetch(
      "https://generous-tuesday.labado.bizml.ru/api/v1/photos",
      {
        method: "POST",
        body,
      }
    ...
    localStorage.setItem("photoUrl", myJson.url);

    const r = await fetch(
      `https://generous-tuesday.labado.bizml.ru/api/v1/donations/${localStorage.getItem(
        "generousId"
      )}/photo`,
      {
        method: "POST",
